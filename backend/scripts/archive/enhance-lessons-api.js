/**
 * Enhanced Lessons Enhancement Script using API
...
const MODULE_ACTIVITY_MAPPING = {
  ...
  "Inkonsonanti": ACTIVITY_TEMPLATES.vowels,
  "Amagambo": ACTIVITY_TEMPLATES.vowels,
  "Ibindi": ACTIVITY_TEMPLATES.vowels
};
...
function createEnhancedActivities(lesson, module) {
  ...
  const activities = baseActivities.map((template, index) => ({
    ...
    order: index + 1,
    estimatedTime: template.points * 2,
    adaptiveContent: {
      hints: {
        available: 3,
        used: 0,
    ...
    for (const lesson of lessons) {
      try {
        ...
        const hasValidActivities = currentActivities.length > 0 && 
          currentActivities.some(activity => activity.type && (activity.question || activity.prompt));
        ...
        await fetch(`${API_BASE_URL}/api/admin/lessons/${lesson.id}`, {
          method: 'PUT',
          headers: {
