...
|------|------|-------|--------|
| `lessons/{lessonId}` | ✅ if authenticated | ❌ | Writes via backend admin API only. |
| `practice/{practiceId}` | ✅ if authenticated | ❌ | Same as lessons. |
| `progress/{userId}` | ✅ if `auth.uid == userId` | ✅ if `auth.uid == userId` | Per-user progress doc. |
...
| `users/{uid}/...` | No rule → **deny** | No rule → **deny** | All access via backend API. |
...
## Progress numbers (12, 38, 50, 24%)

All come from **GET /api/progress** (Bearer token). Nothing is hardcoded.

- **completedLessons** = count of `users/{uid}/lessonHistory` where `passed === true`.
- **remainingLessons** = `totalLessons - completedLessons`.
- **totalLessons** = Firestore `lessons` collection size (`orderBy("order")`).
- **24%** = `(completedLessons / totalLessons) * 100` in the app.
